...
- **Checkpoint validation is a STRUCTURAL floor + an injectable provenance seam.** `validateChain(tail, { base, checkpoint })` verifies the checkpoint's hash, genesis shape, `kind:"checkpoint"`, `subject.type:"run"`, the `czap/checkpoint-summary` payload schema, `subject.id`, and that the retained tail advances the HLC past the checkpoint. That structural floor is sufficient for a TRUSTED checkpoint (the single-actor / own-compaction case this feature targets). It does NOT, on its own, prove provenance: `validateChain` lacks the dropped set, so it cannot recompute the summary `content_hash`, and a malicious remote could mint a structurally-valid checkpoint with a fabricated payload and a timestamp just under `tail[0]`. Adversarial callers close that residual vector by injecting `ChainValidationOptions.verifyCheckpoint` — a provenance hook (the injected-capability idiom, mirroring the gauntlet's host-injected oracles) that establishes the checkpoint came from a trusted compactor (e.g. verifies a `Receipt.macEnvelope` signature over the attestation, or recomputes the summary against a locally-held dropped set) and fails `checkpoint_invalid` when it returns `false`. The engine owns the SEAM; the host owns the KEY — so cryptographic provenance is available downstream without pulling signing / key-management into the domain-free core. Absent a verifier, the structural floor stands.
...
- `packages/core/src/dag.ts` (`checkpoint`, `spliceCheckpoint`, `DAG.Checkpoint`), `receipt.ts` (`ChainValidationOptions` incl. the `verifyCheckpoint` provenance seam, the checkpoint-gated genesis predicate, `checkpoint_invalid`), `packages/_spine/core.d.ts` (mirrored types).
