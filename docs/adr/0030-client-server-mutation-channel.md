...
- The wire is JSON, so a patch must reduce to plain JSON values. A `CapSet`'s `levels` is a `Set` (which `JSON.stringify` silently drops to `{}`) and a policy node's `grants` is schema-opaque, so `sendGraphMutation` REFUSES a non-wire-safe patch at the sender (loud) rather than let a policy node corrupt on the way to the server. A faithful CapSet wire contract (Set ↔ sorted array across seal/decode and every `levels.has` consumer) is a follow-up; until then the seam is loud about what it cannot yet carry, never silent.
