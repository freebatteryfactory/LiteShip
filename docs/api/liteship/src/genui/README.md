[**LiteShip**](../../../README.md)

***

[LiteShip](../../../modules.md) / liteship/src/genui

# liteship/src/genui

`liteship/genui` — the curated facade over `@liteship/genui`.

The facade preserves referential identity: every value is a direct re-export
from the existing package owner, never a wrapper or second implementation.

## Interfaces

- [ComponentCatalogInput](interfaces/ComponentCatalogInput.md)
- [RenderFromCatalogOptions](interfaces/RenderFromCatalogOptions.md)

## Type Aliases

- [ComponentCatalog](type-aliases/ComponentCatalog.md)
- [ComponentDef](type-aliases/ComponentDef.md)
- [ComponentPropDef](type-aliases/ComponentPropDef.md)
- [ContentAddress](type-aliases/ContentAddress.md)
- [GeneratedUINode](type-aliases/GeneratedUINode.md)
- [GeneratedUIValidationError](type-aliases/GeneratedUIValidationError.md)
- [RenderFromCatalogResult](type-aliases/RenderFromCatalogResult.md)
- [ValidateGeneratedUIResult](type-aliases/ValidateGeneratedUIResult.md)

## Variables

- [ContentAddress](variables/ContentAddress.md)
- [DEMO\_COMPONENT\_CATALOG](variables/DEMO_COMPONENT_CATALOG.md)

## Functions

- [catalogHash](functions/catalogHash.md)
- [defineComponentCatalog](functions/defineComponentCatalog.md)
- [renderFromCatalog](functions/renderFromCatalog.md)
- [renderHash](functions/renderHash.md)
- [tryParseGeneratedUIChunk](functions/tryParseGeneratedUIChunk.md)
- [validateGeneratedUITree](functions/validateGeneratedUITree.md)
