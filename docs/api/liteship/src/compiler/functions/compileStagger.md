[**LiteShip**](../../../../README.md)

***

[LiteShip](../../../../README.md) / [liteship/src/compiler](../README.md) / compileStagger

# Function: compileStagger()

> **compileStagger**(`lowered`, `opts?`): [`CompiledStagger`](../interfaces/CompiledStagger.md)

Defined in: compiler/dist/stagger-compile.d.ts:26

Compile a lowered stagger graph into native CSS + runtime write plans per child.

## Parameters

### lowered

[`LoweredStagger`](../../motion/interfaces/LoweredStagger.md)

### opts?

#### prefersReducedMotion?

`boolean`

## Returns

[`CompiledStagger`](../interfaces/CompiledStagger.md)
