[**LiteShip**](../../../README.md)

***

[LiteShip](../../../modules.md) / [genui/src](../README.md) / validateGeneratedUITree

# Function: validateGeneratedUITree()

> **validateGeneratedUITree**(`node`, `catalog`): [`ValidateGeneratedUIResult`](../type-aliases/ValidateGeneratedUIResult.md)

Defined in: [genui/src/validate.ts:193](https://github.com/heyoub/LiteShip/blob/main/packages/genui/src/validate.ts#L193)

Validate a generated UI tree against the host catalog. Unknown names / bad props → reject.

## Parameters

### node

`GeneratedUINode`

### catalog

`ComponentCatalog`

## Returns

[`ValidateGeneratedUIResult`](../type-aliases/ValidateGeneratedUIResult.md)
