[**LiteShip**](../../../README.md)

***

[LiteShip](../../../README.md) / [core/src](../README.md) / inputSourceType

# Function: inputSourceType()

> **inputSourceType**(`input`): `"viewport"` \| `"time"` \| `"pointer"` \| `"scroll"` \| `"media"` \| `"custom"` \| `"audio"` \| `undefined`

Defined in: [core/src/reactive/signal-input.ts:141](https://github.com/freebatteryfactory/LiteShip/blob/main/packages/core/src/reactive/signal-input.ts#L141)

The [SignalSourceType](../type-aliases/SignalSourceType.md) family of an input string, or `undefined`.

## Parameters

### input

`string`

## Returns

`"viewport"` \| `"time"` \| `"pointer"` \| `"scroll"` \| `"media"` \| `"custom"` \| `"audio"` \| `undefined`
