[**LiteShip**](../../../README.md)

***

[LiteShip](../../../modules.md) / [core/src](../README.md) / projectionKeys

# Function: projectionKeys()

> **projectionKeys**(`name`): [`ProjectionKeys`](../interfaces/ProjectionKeys.md)

Defined in: [core/src/projection.ts:72](https://github.com/heyoub/LiteShip/blob/main/packages/core/src/projection.ts#L72)

Derive the [ProjectionKeys](../interfaces/ProjectionKeys.md) for a quantizer/satellite name.

## Parameters

### name

`string`

## Returns

[`ProjectionKeys`](../interfaces/ProjectionKeys.md)
