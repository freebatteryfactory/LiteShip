...
[LiteShip](../../../README.md) / [core/src](../README.md) / assertTokenBinds

# Function: assertTokenBinds()

> **assertTokenBinds**\<`T`\>(`proposal`): `T`

Defined in: [core/src/evidence/validated-output.ts:141](https://github.com/freebatteryfactory/LiteShip/blob/main/packages/core/src/evidence/validated-output.ts#L141)
...
It enforces, at runtime, the same three properties the type encodes:
 1. PROVENANCE — the token carries the module-private witness, so it was minted
    by [mintValidated](https://github.com/freebatteryfactory/LiteShip/blob/main/packages/core/src/evidence/validated-output.ts) (a runtime brand check that backs the type-level
