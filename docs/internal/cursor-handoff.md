...
2. **#132 — completeness gate, NARROW first.** Field-level active-surface-has-reader, red-fixtured against the *live* `TransitionNode` orphan; reuse `@liteship/audit` orphan facts + `@liteship/gauntlet` authority ratchet (no `typescript` dep in gauntlet). Prove it bites on one surface, then generalize. This gate defines "done" for #130.
...
- **Don't delete the "dead" `liteship:request-snapshot` dispatch or `ResumptionConfig.timeout`** (#122) — unfinished features, not cruft. Complete them (wire `request-snapshot` → `refreshBase`/`adopt`; thread `timeout` → `AbortSignal`).
...
**Secondary — resolved.** StateCell is NOT required for #124 "done" (pose + `PolicyNode` + SSR receipt #118 + interpreter + CSS + floor is enough; StateCell is child 5, a follow-on). No `StaggerIntent` in the first slice (one reveal only, §3e). #112 (from-parts builder) is a *soft* prereq — land it with/just-before the motion slice's node construction; interim, seal nodes the fixture way. `writeContinuousMap` dispatches both: `detail.css` (typed property map) always + `detail.wgsl`/`glsl` (`liteship:uniform-update`) for GPU-bound props only — preserving the leaf-write law (never a per-frame patch).
