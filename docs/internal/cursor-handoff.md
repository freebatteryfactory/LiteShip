...
3. **Graph wiring — PROPOSED (the one decision to confirm before interpreter code).** A `TransitionNode` binds to a boundary **transitively, no new field:** `TransitionNode.fromPose → PoseNode.entityRef → EntityNode → ComponentNode.boundaryRef`; both poses are states of the same entity's boundary. `interpretTransition` (the reader `lowerGraph` lacks) walks TransitionNodes: resolves the boundary via that chain, diffs `fromPose.bindings`/`toPose.bindings` → typed (from,to) per property, maps `fromPose.state`/`toPose.state` to the boundary's discrete crossing (the trigger), and `routing`/`durationMs` to keyframe sequencing/timing → emits a `css` projection plan (keyframes/transition keyed on `data-czap-state`) + a runtime leaf-write plan on the same crossings. Multi-step (seq/par/choice) sequences via `EdgeType`. **Confirm:** transitive binding (recommended — graph-native, reuses `entityRef`, no denormalization) vs. a direct `TransitionNode.boundaryRef`/selector field.
