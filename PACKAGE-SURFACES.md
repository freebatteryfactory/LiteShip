...
Exact named bindings, producer ownership, audience, stability, and proof status
are enforced by `public-exports:check` and indexed in
[PUBLIC-EXPORTS.md](./PUBLIC-EXPORTS.md).
...
| --- | --- | --- | --- |
| `@liteship/_spine` | foundation | `.`, `./core`, `./design`, `./command`, `./genui`, `./events` | shared-type-declarations |
...
| `@liteship/core` | semantic | `.`, `./authoring`, `./reactive`, `./motion`, `./graph`, `./evidence`, `./schema`, `./media`, `./clock`, `./wasm`, `./harness`, `./simulation`, `./ecs`, `./fs-walk` | adaptive-authoring, reactive-runtime, motion, document-graph, evidence |
...
| `@liteship/vite` | host-adapter | `.`, `./html-transform`, `./hmr`, `./virtual` | vite-integration, css-directive-compilation, hmr |
...
The semantic and runtime foundation. Runtime status: `host-wired`. The normal root stays focused on authoring and shared runtime primitives; the typed projection/execution substrate is an explicit advanced subpath at `@liteship/core/ecs`.
...
### Typed ECS (`@liteship/core/ecs`)

Use this advanced subpath for continuous, tick-driven projections that must not re-seal the document graph on every frame. Component identity is a minted `Part<T>`, unknown values cross through `admitPart`, systems declare their query/read/write authority, and dense-store mutation stays owner-only. Free-string component names and unadmitted world writes are not part of the contract. Scene is the primary consumer; other hosts must justify the same continuous-execution seam rather than treating ECS as a second state model.

Main surfaces: `definePart`, `admitPart`, `defineSystem`, `defineDenseSystem`, `createWorld`, `createDenseStore`, `Part`, `System`, `DenseSystem`, `World`.
...
- `CapabilityTierEvidence` / `projectCapabilityTierEvidence` — complete tier values paired with per-axis observed/inferred provenance
- `requireObserved` — fail-closed admission for decisions that require fully observed capability axes
...
`EdgeTier.detectTier()` returns a complete conservative `capTier`/`motionTier`/`designTier` triple plus `tierEvidence`. Rendering and cache identity use the complete values; callers that require measured inputs admit specific axes through `@liteship/detect`'s `requireObserved` rather than collapsing provenance into one aggregate confidence number.
