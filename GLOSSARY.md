...
| **Signal source** | The canonical vocabulary for what drives a boundary: `SignalSource` (the typed union in `@czap/core`) is the source of truth, and `sourceToInput` / `inputToSource` round-trip the dot-string input form (`viewport.width`, `scroll.progress`, `audio.amplitude` / `audio.beat`). Every domain reads through it rather than re-parsing input strings. |
