 ...
 * is likewise not trusted to prove zero runtime — `zero-runtime.mjs` emits and
...
export const exportsALaw = (/** @type {string} */ text) => /=\s*Assert</u.test(text);

/** Whether one emitted declaration imports a module that is compile-only. */
export const importsACompileOnlyModule = (/** @type {string} */ text) =>
  /from\s+['"][^'"]*\.(laws|type-test)\.js['"]/u.test(text);

const emit = (/** @type {string} */ out) => {
  execFileSync(
    process.execPath,
...
if (import.meta.main) {
  ...
  try {
    ...
    const emitted = walk(first).filter((file) => file.endsWith('.d.ts'));
    const findings = [];

    // An empty population passes every check below it.
    if (emitted.length === 0) {
    ...
    for (const file of emitted) {
      const name = relative(first, file).replaceAll('\\', '/');
      const text = readFileSync(file, 'utf8');

      if (name.includes('.laws.') || name.includes('.type-test.')) {
        findings.push(`compile-only module in the public surface: ${name}`);
      }
      if (exportsALaw(text)) findings.push(`law alias exported from: ${name}`);
      if (importsACompileOnlyModule(text)) findings.push(`imports a compile-only module: ${name}`);
    }
