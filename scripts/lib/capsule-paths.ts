 ...
 * `CZAP_CAPSULE_GENERATED_DIR` so parallel tests can redirect their compile
 * output to a per-test temp dir (CUT T1). This is the generated-dir twin of
 * `CZAP_CAPSULE_MANIFEST` / `getCapsuleManifestPath` — together they let a
...
/** Resolve the generated test/bench output dir, honoring `CZAP_CAPSULE_GENERATED_DIR`. */
export function getCapsuleGeneratedDir(cwd: string = process.cwd()): string {
  const raw = process.env.CZAP_CAPSULE_GENERATED_DIR?.trim();
