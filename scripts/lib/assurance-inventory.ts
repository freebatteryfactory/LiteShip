...
export interface AssuranceRegressionOptions {
  /**
   * Require addressed live mutation and MC/DC receipts. Continuous/quick
...
export function buildAssuranceInventory(cwd: string, options: AssuranceInventoryOptions = {}): AssuranceInventory {
  ...
  // Generated projections have their own freshness/parity authorities. Charging
  // their mechanical LOC to authored-evidence density would reward duplicating
  // their generated bytes with equally mechanical tests rather than testing the
  // generator and projection law once.
  const sourceLoc = Object.entries(sourceRoles).reduce(
    (sum, [role, value]) => sum + (role === 'generated' ? 0 : value),
    0,
  );
...
export function assuranceRegressions(
  ...
  const semanticGaps = options.requireSemanticAssurance
    ? inventory.packages.flatMap((entry) =>
        entry.missingEvidence
          .filter((evidenceGap) => CAMPAIGN_EVIDENCE.has(evidenceGap))
          .map((evidenceGap) => ({
            package: entry.name,
            kind: 'evidence-gap' as const,
            evidenceGap,
          })),
      )
    : [];
  return [...density, ...gaps, ...semanticGaps].sort(
    (left, right) =>
