...
const ERROR_CODES_REL = 'packages/error/src/codes.ts';
...
/** Project check diagnostics from the canonical registry into the dependency-leaf error package. */
function renderCheckDiagnostics(): string {
  return CHECK_REGISTRY.map(
    (entry) =>
      `  // prettier-ignore -- generated byte projection from CHECK_REGISTRY\n` +
      `  ${tsString(entry.id)}: check(\n` +
      `    ${tsString(entry.title)},\n` +
      `    ${tsString(entry.claim)},\n` +
      `    ${tsString(entry.remediation)},\n` +
      '  ),',
  ).join('\n');
}
...
const CODE_PROJECTIONS = [[ERROR_CODES_REL, 'CHECK-DIAGNOSTICS', renderCheckDiagnostics]] as const;
...
function applyCodeProjection(source: string, marker: string, rendered: string): string {
  const begin = `  /* BEGIN gen-roster: ${marker} */`;
  const end = `  /* END gen-roster: ${marker} */`;
  const beginAt = source.indexOf(begin);
  const endAt = source.indexOf(end, beginAt + begin.length);
  if (beginAt === -1 || endAt === -1) throw new Error(`gen-roster: missing ${marker} markers`);
...
function write(): number {
  ...
  for (const [relativePath, marker, render] of CODE_PROJECTIONS) {
    const path = resolve(REPO_ROOT, relativePath);
    const source = readFileSync(path, 'utf8');
    writeIfChanged(relativePath, applyCodeProjection(source, marker, render()));
  }
...
export function collectGeneratedProjectionDrift(
  ...
  for (const [relativePath, marker, render] of CODE_PROJECTIONS) {
    const source = readProjection(relativePath);
    if (source == null || applyCodeProjection(source, marker, render()) !== source) {
      drift.push({ copy: relativePath, detail: `stale generated ${marker} projection` });
    }
  }
