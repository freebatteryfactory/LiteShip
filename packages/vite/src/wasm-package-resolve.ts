/**
 * Locate the `liteship-compute.wasm` artifact shipped inside `@liteship/core`.
 *
 * Resolved through THIS plugin's own dependency edge — `@liteship/vite` depends on
 * `@liteship/core` — via `import.meta.url`, NOT the consumer's project root. That is
 * the canonical pnpm-nesting-safe "a package finds its dependency's asset"
 * pattern: it works even when the app installs only `@liteship/astro`/`liteship` and
 * pnpm's strict linker keeps `@liteship/core` out of top-level `node_modules`, and it
 * resolves the EXACT `@liteship/core` this `@liteship/vite` was built against (so the
...
/** Where the artifact sits inside a built/installed `@liteship/core`. */
const CORE_WASM_DIST_PATH = 'dist/liteship-compute.wasm';
...
function packageRootFrom(entryPath: string, pkgName: string): string | null {
  ...
  for (let depth = 0; depth < 10; depth++) {
    ...
    if (fileExists(manifest, 'liteship/vite.wasm-resolve')) {
      try {
        ...
        const skippable =
          ...
          code === 'ENOTDIR';
        if (!skippable) {
          throw IoError('liteship/vite.wasm-resolve', `unreadable package manifest while walking to ${pkgName}`, {
            path: manifest,
            cause: err,
          });
        }
...
/**
 * Resolve `@liteship/core`'s shipped `dist/liteship-compute.wasm`, or `null` when
 * `@liteship/core` (or its wasm) is absent — a 0.2.0 install, or none — so callers
 * fall through to the next WASM source.
 */
export function resolvePackagedWasm(): string | null {
  let coreEntry: string;
  try {
    // ESM resolution (import.meta.resolve), NOT createRequire: @liteship/core's
    ...
    // (allowlisted in @liteship/audit policy).
    coreEntry = fileURLToPath(import.meta.resolve('@liteship/core'));
  } catch {
    return null;
  }
  const coreRoot = packageRootFrom(coreEntry, '@liteship/core');
  if (coreRoot === null) return null;
  const wasm = path.join(coreRoot, CORE_WASM_DIST_PATH);
  return fileExists(wasm, 'liteship/vite.wasm-resolve') ? wasm : null;
}
