...
/** Map diagnostic nouns to factory functions for teaching errors. */
...
/** Map `_tag` values to factory functions for teaching errors. */
const TAG_TO_FACTORY: Record<string, string> = {
  BoundaryDef: 'defineBoundary',
  TokenDef: 'defineToken',
  ThemeDef: 'defineTheme',
  StyleDef: 'defineStyle',
};
  ...
  if (exported && typeof exported === 'object' && '_tag' in exported && exported._tag !== expectedTag) {
    Diagnostics.warn({
      ...
      message:
        ...
        `Fix: create it with the factory — \`export const ${exportName} = ${TAG_TO_FACTORY[expectedTag] ?? NOUN_TO_FACTORY[diagnosticNoun] ?? 'theCorrectFactory'}({ ... })\` ` +
