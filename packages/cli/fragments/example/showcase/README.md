...
- **`/`** — the CSS state mirror: one `layout` boundary drives the grid, and the
  status readout renders purely off `data-liteship-state`. Resize and watch both move
...
- **`/worker`** — boundary evaluation off the main thread over an SPSC ring
  ...
  `data-liteship-state` — flipping on resize while the main thread stays free.
...
- **`/responsive-media`** — the responsive-media effective-candidate cookbook (#140):
  ONE authored `ResponsiveMedia.intent` projected by `Astro.locals.liteship.responsiveMedia`,
...
- **`/chat`** — `client:llm` streaming over `/api/chat` with tier-gated
  rendering, plus the generated-UI path (`data-liteship-genui`): `_genui` chunks
