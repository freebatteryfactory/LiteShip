// Ambient declaration for @liteship/mcp-server, used only by the ONE-WAY dynamic
// import in dispatch.ts (the `mcp` subcommand launches the server).
//
// CUT A1 capstone: the cli↔mcp CYCLE is gone — `@liteship/mcp-server` no longer
// imports `@liteship/cli` (it dispatches through @liteship/command). This is the lone
// remaining edge, cli→mcp, and it's a deliberately-exempt dynamic import (the
// A1-T3 detector allows it). `@liteship/cli` does not project-reference mcp-server,
