 ...
 * shipped: `example` (the runnable/dogfood apps under `examples/`) and `template`
 * (the `create-liteship` starter templates). A fragment is copied verbatim into
...
import { findWorkspaceRoot } from './doctor/manifest.js';

/** The scaffold-fragment kinds this minimal copier understands. */
export type FragmentKind = 'example' | 'template';

/** Workspace-relative source directory that holds each kind's fragments. */
const FRAGMENT_ROOTS: Record<FragmentKind, string> = {
  example: 'examples',
  template: 'packages/create-liteship/templates',
};
 ...
 * fragments (exit 0). With `kind`+`name`, copies `examples/<name>` or the
 * template into `./<name>` and reports the copied file count. Exit 1 on an unknown
 * kind/name or an already-existing destination.
 */
export async function add(opts: { kind?: string; name?: string; cwd?: string } = {}): Promise<number> {
  const cwd = opts.cwd ?? process.cwd();
  const root = findWorkspaceRoot(cwd);
  const available = listFragments(root);
  ...
  const source = resolve(root, FRAGMENT_ROOTS[opts.kind], opts.name);
