/**
 * The authored SPINE-RELATION admission seed — the LiteShip-LOCAL, host-owned input the two-axis
...
export const LITESHIP_SPINE_EXACT_RELATION_CATALOG: readonly SpineExactRelationCatalogEntry[] = [
  ...
  },
  {
    ...
    relations: [
      ...
      { typeName: 'AnimatedQuantizer', spineExpr: 'AnimatedQuantizer<any>' },
  ...
  },
  {
    ...
    relations: [
      ...
      'PrimitiveKind',
      {
        typeName: 'VitePrimitive',
        spineExpr: "VitePrimitive<'boundary' | 'token' | 'theme' | 'style'>",
        runtimeExpr: "Primitive<'boundary' | 'token' | 'theme' | 'style'>",
      },
      {
...
export const LITESHIP_SPINE_AUTHORED_ADMISSIONS: readonly SpineTypeAdmission[] = [
  ...LITESHIP_SPINE_EXACT_RELATION_CATALOG.flatMap(exactRelations),

  // `StateUnion<any>` evaluates to `any` by definition and would make a generic
  // relation probe vacuous. Pin one concrete Boundary instantiation instead.
  runtimeMirror(
    'StateUnion',
    `${CORE}/authoring/types.ts`,
    'exact',
    "StateUnion<{ readonly _tag: 'BoundaryDef'; readonly _version: 1; readonly id: any; readonly input: any; readonly thresholds: readonly any[]; readonly states: readonly ['spine-probe']; readonly hysteresis?: number; readonly spec?: any }>",
  ),
