...
function assuranceElements(repoRoot: string): readonly StandardsElement[] {
  const policy = LITESHIP_ASSURANCE_MAP.map((rule, order): StandardsElement => ({
    _tag: 'assurance',
    glob: rule.glob,
    level: rule.level,
    order,
  }));
  const coverage = assuranceSubjects(repoRoot).map((file): StandardsElement => ({
...
/**
 * Enumerate the live source subjects whose EFFECTIVE assurance must not fall.
 * Rule strings are policy syntax; these paths are the governed reality. Keeping
 * the census host-owned preserves the lean engine's no-filesystem boundary.
 */
function assuranceSubjects(repoRoot: string): readonly string[] {
  ...
  for (const root of roots) {
    ...
    for (const file of walkFiles(absolute, {
      skipDirs: ['node_modules', 'dist', 'coverage', '.cache'],
      extensions: ['.ts', '.tsx', '.mts', '.cts', '.js', '.mjs', '.cjs'],
    })) {
      out.push(relative(repoRoot, file).replaceAll('\\', '/'));
    }
  }
...
export function readLiveStandardsSurface(repoRoot: string, now: Date): StandardsSurface {
  ...
  return { snapshotFormat: 2, elements, address };
}
...
function parseSnapshot(raw: string, source: string): StandardsSurface {
  ...
  if (
    !isRecord(parsed) ||
    (parsed.snapshotFormat !== 1 && parsed.snapshotFormat !== 2) ||
    !Array.isArray(parsed.elements)
  ) {
    throw ParseError(source, 'not a recognizable standards snapshot (snapshotFormat 1|2 + elements[])');
  }
  return {
