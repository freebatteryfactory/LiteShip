...
```bash
pnpm add @liteship/cloudflare @liteship/astro @astrojs/cloudflare@^14 astro@^7 wrangler@^4
```
...
export const onRequest = cloudflareMiddleware({
  binding: 'LITESHIP_BOUNDARY_CACHE',   // KV namespace name in wrangler.jsonc
  manifest: boundaries,
  boundary: 'viewport',             // optional when the manifest has one entry
});
```

Each request now resolves the visitor's device tier and serves that tier's precompiled boundary CSS through `Astro.locals.liteship` — `locals.liteship.edge.cacheStatus` reads `precompiled` or `hit` once KV is wired. `virtual:liteship/boundaries` is the manifest the `@liteship/vite` plugin derives at build time: each entry carries the boundary's minted content address (a hash of its definition) plus per-tier outputs, so nothing is hand-typed. For editor types, add `/// <reference types="@liteship/vite/virtual" />` to `src/env.d.ts`.

For Astro 7 route-cache invalidation, use `@liteship/cloudflare/cache-provider` in `astro.config.mjs` and pass matching `tags` to `cloudflareMiddleware()`. `cache.invalidate({ tags })` then purges the same KV tag index that boundary compile fallbacks write.

Because `cloudflareMiddleware` wraps `@liteship/astro`'s `liteshipMiddleware`, the Workers edge also gets the responsive-media host projection (#140): `Astro.locals.liteship.responsiveMedia(intent)` derives Save-Data / DPR caps from the request's Client Hints and projects through `@liteship/core`'s `selectCandidates` law, so a Save-Data client on the edge is never advertised a heavy image candidate through `src` / `srcset` / `<source>` / the preload; the responsive `Vary` axis (`Sec-CH-DPR, Save-Data`) is merged into the response. Runnable in `examples/cloudflare-astro`.

## Where it sits

A host adapter — it touches Cloudflare APIs (the `cloudflare:workers` env and KV namespaces) so nothing else has to. It depends on [`@liteship/astro`](https://github.com/freebatteryfactory/LiteShip/tree/main/packages/astro) for the middleware contract, [`@liteship/edge`](https://github.com/freebatteryfactory/LiteShip/tree/main/packages/edge) for tier detection and the content-addressed cache, and [`@liteship/core`](https://github.com/freebatteryfactory/LiteShip/tree/main/packages/core) for the id types. Boundary authoring and compilation live upstream; this package is the Cloudflare glue for middleware, Workers KV, and Astro 7 cache invalidation. See the
[package surfaces map](https://github.com/freebatteryfactory/LiteShip/blob/main/PACKAGE-SURFACES.md)
for the full layout.

## If it does nothing

A mistyped `binding` emits a LiteShip diagnostic, KV reads return null, writes no-op, and every request falls back to the manifest or `compile` path with `cacheStatus: 'miss'`. Check that `binding` matches the KV namespace name in `wrangler.jsonc`, then run `liteship doctor --target cloudflare` (from `@liteship/cli`) for a preflight of Astro, adapter, and wrangler config.
