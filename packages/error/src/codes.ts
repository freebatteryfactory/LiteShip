...
export const DIAGNOSTIC_REGISTRY = Object.freeze({
  ...
  /* BEGIN gen-roster: CHECK-DIAGNOSTICS */
  // prettier-ignore -- generated byte projection from CHECK_REGISTRY
  'check/format': check(
    'Prettier formatting',
    'Every Prettier-supported package, fragment, bin, config, test, and script source is formatted.',
  ...
  'check/rustfmt': check(
    ...
    "run 'pnpm exec tsx scripts/rustfmt-check.ts --write' with the repository toolchain, then re-run.",
  ),
  // prettier-ignore -- generated byte projection from CHECK_REGISTRY
  'check/rust-wasm-qualification': check(
    'Pinned Rust and WASM qualification',
    ...
    'fix the named pinned Clippy or WASM build arm, then re-run.',
  ),
  // prettier-ignore -- generated byte projection from CHECK_REGISTRY
  'check/cargo-audit': check(
    'Pinned Rust dependency advisory audit',
  ...
  // prettier-ignore -- generated byte projection from CHECK_REGISTRY
  'check/security-minimum': check(
    'Security-sensitive dependency floors',
    'Every governed security override and resolved lockfile instance meets its fixed minimum version.',
    'raise the canonical override to the governed fixed version and regenerate pnpm-lock.yaml.',
  ),
  // prettier-ignore -- generated byte projection from CHECK_REGISTRY
  'check/security-audit': check(
    'Live package-registry vulnerability audit',
    'The current lockfile has no high- or critical-severity advisory in the live pnpm registry receipt.',
  ...
  'check/lint': check(
    ...
    'Package, fragment, bin, config, test, and script source passes ESLint with zero warnings.',
  ...
  'check/typecheck': check(
    ...
    'fix the native TypeScript errors in the build, scripts, tests, or shipped-source contexts.',
  ),
  // prettier-ignore -- generated byte projection from CHECK_REGISTRY
  ...
  /* END gen-roster: CHECK-DIAGNOSTICS */
