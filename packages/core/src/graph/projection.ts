 ...
 * `@liteship/compiler`'s GLSL arm); {@link wgslIdent} is its WGSL twin (the bare
...
function projectionKeys(name) {
  ...
  return { cssKey: "--liteship-" + name, glslKey: "u_" + snake, wgslKey: snake, ariaKey: "data-liteship-" + name };
}`;
