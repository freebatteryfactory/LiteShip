...
const noFixme = defineGate({
  id: 'acme/no-fixme',
  extension: { namespace: 'acme', owner: '@acme/quality' },
  level: 'L2',
  ...
  run: (ctx) =>
    ctx
      ...
      .map((f) =>
        finding({
          ruleId: 'acme/no-fixme',
          severity: 'error',
          level: 'L2',
          title: 'FIXME found',
          detail: `${f} still carries a FIXME.`,
          location: { file: f },
        }),
      ),
  fixtures: {
...
`runGates` first verifies each gate against its own fixtures: the red must flag, the green must pass clean, and the mutation must break one of them. A gate that proves itself can block on its semantic errors. One that cannot is demoted to `advisory` for those semantic findings, but the failed qualification emits a separate blocking `gauntlet/authority-integrity` finding. A required gate with an unverified detector can therefore never mint green. Compose your own gates alongside `LITESHIP_GATES` — the same ratchet qualifies all of them.
...
The authority ratchet is the usual surprise: a gate whose semantic findings stay `advisory` has not self-proven. Check its fixtures — the `red` must produce at least one finding, the `green` must produce none, and the `mutation` must make one of those fail. The gate may not use unqualified logic to block source, but its failed qualification blocks the run through `gauntlet/authority-integrity`; repair the proof rather than waiving or ignoring it.
