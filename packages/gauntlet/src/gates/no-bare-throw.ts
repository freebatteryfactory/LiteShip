 ...
 * a tagged `@liteship/error` variant, never a bare `throw new Error(...)`. It ships
      ...
      if (BARE_THROW.test(lines[i] ?? '')) {
        findings.push(
          finding({
            ...
            detail: `${file}:${i + 1} throws a bare Error. Every failure path must be a tagged @liteship/error variant so it carries a _tag, structured fields, and a catchable identity.`,
