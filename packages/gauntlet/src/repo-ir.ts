...
/**
 * The stronger of two coverage classes by {@link COVERAGE_CLASSES} ordering — the
 * higher-confidence oracle's class, which a divergence finding carries (so the
 * reader sees the strongest evidence behind it).
 */
export function strongerCoverageClass(a: CoverageClass, b: CoverageClass): CoverageClass {
  return COVERAGE_CLASSES.indexOf(a) >= COVERAGE_CLASSES.indexOf(b) ? a : b;
}
