/**
 * Surface audit pass (CUT D9b-1, relocated from scripts/audit) — package-export
 * resolvability + host-surface inventory (Astro directives/runtime, Vite virtual
 * modules). Profile-driven (CUT D9a): host-surface checks are owned by
 * `profile.surfacePolicy`; an empty policy (a downstream project with no Astro/
 * Vite host) carries no LiteShip assumptions.
 *
 * @module
 */
