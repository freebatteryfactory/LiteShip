...
export function mutantVerdictKey(
  ...
  coveringTestDigest?: (testId: string) => string,
): string {
  const sorted = [...coveringTests].sort((a, b) => a.localeCompare(b));
  const entries = coveringTestDigest === undefined ? sorted : sorted.map((id) => [id, coveringTestDigest(id)]);
  const coveringDigest = addressedDigestOf(CanonicalCbor.encode(entries), 'blake3').integrity_digest;
...
function cacheKeyFor(
  ...
  return mutantVerdictKey(mutant, coveringTests, options.toolchainDigest, options.coveringTestDigest);
}
