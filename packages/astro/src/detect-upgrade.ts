 ...
 * it CANNOT import `@liteship/detect` at runtime. Earlier releases re-typed the
 ...
 * `@liteship/detect` by `emitDetectUpgradeScript`, which folds the head probe's
...
import { emitDetectUpgradeScript } from '@liteship/detect';

/**
 * Inline GPU-probe script, DERIVED from canonical `@liteship/detect`. Injected via
