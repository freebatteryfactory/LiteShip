 ...
 *
 * Samples the ONE shared kernel `sampleProgram` (`@czap/core`, Law 4) for progress
 * `t` and projects the result to the DOM: sets each typed CSS custom property and
 * dispatches `czap:uniform-update` with `detail.css` (all props) plus `detail.wgsl`
...
import { sampleProgramUniforms, type RuntimeWritePlan } from '@czap/core';
 ...
 * `sampleProgramUniforms` (`@czap/core`); this function owns only the DOM effects: it writes each
 * CSS custom property on `el` and dispatches one `czap:uniform-update` carrying
