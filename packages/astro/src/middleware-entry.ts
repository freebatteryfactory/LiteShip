 ...
 * channel and populates `Astro.locals.czap` from Client Hints — so a consumer
 ...
 * still adds their own `src/middleware.ts` calling `czapMiddleware({ edge })`.
 * That one runs AFTER this `order: 'pre'` entry and refines the same locals.
 *
 * @module
 */
import type { MiddlewareHandler } from 'astro';
import { czapMiddleware } from './middleware.js';

/**
 * Astro middleware that runs zero-config capability detection, populating
 * `Astro.locals.czap`. Auto-wired by the integration via `addMiddleware` when
 * `czap({ middleware: true })`. Annotated as Astro's `MiddlewareHandler` so the
 * declaration emit doesn't leak the package-internal `MiddlewareContext` (TS4023).
 */
export const onRequest = czapMiddleware() as unknown as MiddlewareHandler;
