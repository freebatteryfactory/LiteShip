 ...
 * `Cross-Origin-Opener-Policy` / `Cross-Origin-Embedder-Policy` pair liteship emits so
 * `SharedArrayBuffer`-backed workers (`@liteship/worker`'s SPSC ring) get cross-origin
 * isolation.
 *
 ...
 *  - `@liteship/astro`'s `CROSS_ORIGIN_HEADERS` builds itself from {@link CrossOriginIsolation.isolationHeaders};
 *  - `liteship doctor --deployed`'s live header probe validates a deployed response's
...
/**
 * COEP values liteship can emit; both establish cross-origin isolation. `require-corp`
...
/** The COEP value liteship emits by default (consumer-overridable to `credentialless`). */
