...
## 25. Machine-checkable architecture projection

```yaml
artifact:
  name: liteship-core-architecture
  status: confirmed-closed
  implementation: absent
  authorities:
    architecture: README.md
    topology: types.ts
    local: <numbered-home>/README.md + <numbered-home>/types.ts
repository_as_spec:
  required_home_files:
  - README.md
  - types.ts
  dependency_authority: source-imports-plus-numbered-ceiling
  generated_status_documents: false
  duplicate_dependency_lists: false
  duplicate_maturity_rollup: false
  path_enforcement: required
  type_abi_enforcement: required
naming:
  established_terms: required
  branded_synonyms_as_primary_api: false
  generic_drawers: false
failure_model:
  authored_definition_failure: structured-throw-over-one-result-validator
  external_data_failure: result
  operation_failure: result
  wire_failure: result-projection
  invariant_failure: structured-throw
  machine_diagnostic: lossless
  default_human_diagnostic: summary-location-cause-first-safe-action
production:
  unresolved_holes: blocking
  unresolved_recursive_references: blocking
  unresolved_runtime_features: blocking
  deferred_validation: forbidden
  deferred_trust: forbidden
  hidden_load_registration: forbidden
  opaque_thunks_in_canonical_data: forbidden
  explicit_operational_deferral: allowed
  explicit_any: forbidden
waterfall:
  import_rule: root-self-or-earlier-home
  topology_file_is_barrel: false
  canonical_owner_imports: required
  home_order:
  - 00_error
  - 01_encoding
  - 02_identity
  ...
  - 18_inspection
references:
  generic_carrier: DataPath
  semantic_owner: schema
  typed_field_reference: true
  named_reference_traversal: explicit-compatible-composition
  entity_field_product: EntityFieldReference
  raw_string_as_semantic_field: false
  universal_expression_language: false
records:
  universal_receipt_term: false
  type_abi_evidence: attestation
  terms:
  - address
  - revision
  - event
  ...
  - receipt
schema:
  native_core_authority: true
  explicit_any_node: false
  unknown_fields_default: reject
  recursion: named-definition-and-reference
  field_navigation: direct-finite-properties-plus-collision-safe-view-and-explicit-reference-composition
  typed_schema_references_preserve_port: true
  faithful_projection_or_explicit-refusal: true
evidence:
  truth: strong-kleene
  pending_is_unknown: false
  collection_predicates_reuse_propositions: true
operation:
  model: delegated-business-effect
  approval_source: operation-policy
  resource_budget: independent-named-limits-with-policy-derived-effective-budget
  separate_agent_language: false
state:
  semantic_authority: core
  durable_persistence: host-ports
  change_flow: operation-to-family-patch-to-normalized-change-to-commit
  ecs_realizations:
  - semantic-revision
  - dense-data
  - scene
  - interaction
  - future-replicated
  nested_composition: addressed-subworlds
  dense_slot_is_public_identity: false
  component_type_relation: schema-reference-and-state-follow-one-port
quantization:
  interpolation_owner: 09_quantization
  scene_local_easing_language: false
collection:
  rows_live_in: WorldRevision
  algebra: typed-relational
  query_field_scope: declared-row-roots
  predicate_truth: strong-kleene
  deterministic_final_tie_breaker: row-key
  collation: content-addressed-explicit-profile
scene:
  semantic_authority: declarative-scene
  spatial_algebra: dedicated-typed-coordinate-projection
  transform_order: explicit
  parent_origin_authority: to-parent-transform
  coordinate_bearing_geometry_is_space_typed: true
  material_color_space_explicit: true
  timeline_interpolator: quantization-reference
  timeline_field_target: entity-plus-schema-field
  geometry_support_requires:
  - bounds
  - transform-behavior
  - per-egress-exactness-tolerance-and-fallback
  - interpolation-or-refusal
  highest_risk_proof_area: spatial-algebra
stream:
  one_envelope_family_specific_payloads: true
  trusted_fragment_generated_structure_separation: required
  payload_rosters: exact-and-non-overlapping
  attestation_reference_kinds: distinct
  trusted_fragment_patch: constrained-semantic-location-family
  frame_classes:
  - snapshot
  - patch
  - hold
  - prediction
compiler:
  legality_before_cost: true
  planner: monotone-constraint-propagation-plus-bounded-cost-selection
  optimizer_is_quantizer: false
  runtime_bundle_source: residual-requirement-closure
  runtime_feature_requirements: exact-hole-derived
  fixed_hydration_tiers: false
  assurance_solver_oracle: allowed
program:
  semantic_form: ResidualProgram
  logical_memory_form: MemoryPlan
  physical_layout_form: MemoryLayout
  packed_form: ExecutionImage
  versioning: root-envelope
  reserved_key_fixtures: compatible-tag-and-version-shadows
  optional_bytecode: evidence-earned
runtime:
  semantic_reference: typescript
  general_kernel_abi: true
  backend_profiles: content-addressed-measurements
  backend_selection_boundary: program-or-segment
  output_types_follow_schema_through_commit: true
  backends:
  - html-css
  - javascript
  - wasm
...
editor:
  ...
  - history-cursor
  preview_reference_kind: draft-revision
  approval_source: operation-policy
  proposal_operation_owner: invocation
  human_agent_operation_model: shared
research:
  bytecode_default: deferred
  broad_gpu_reconciliation: deferred
  replication_algorithm: deferred
  dedicated_file_format: deferred-until-editor-trigger
proof:
  capability_not_file: true
  type_negative_fixtures_required: true
  reachability_required: true
  measured_body_must_match_claim: true
next_action:
  required: remaining-host-children-then-targets-wires-system
  implementation_before_phase_opens: false
```
