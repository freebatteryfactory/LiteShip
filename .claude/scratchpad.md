...
### CORRECTION (owner caught me littering) — SUPERSEDES all "@czap/artifacts / @czap/path / new package / Wave B = extract into a package / Slice B repo-IR" language above
I proposed NEW PACKAGES (@czap/artifacts, @czap/path) and a future-shaped Slice-B repo-IR. That is LITTER + placeholders for future consumers — forbidden. NONE of it was ever code (only notes); all such proposals DELETED from decisions-pending.
THE LAW (re-anchored): nothing enters the repo unless it is a COMPLETE, EXERCISED organ — real producer + real consumer + red/green fixture + a gate/test proving it, IN THE SAME CHANGE. Future Slice C is not a consumer. "Clean abstraction" is not a justification. No new packages — we INTEGRATE into what exists. No OOP interface gardens (use discriminated unions / plain data / pure functions / capability records). No reserved fields. Plan-only ideas stay in the gauntlet PLAN file, never as repo code or pseudo-decisions.
- The parked gates (runtime-gate/feedback-verify/devx-check/…) and the artifact-* lib cluster: when they move, they INTEGRATE into an EXISTING package (the artifact/audit logic belongs in @czap/audit; bench logic stays a command lib) — NO new package, and only when it lands as a complete organ.
- check-invariants CLI-only was a NERF based on a wrong "needs a new @czap/path package" conclusion. It can stay MCP-exposed by INTEGRATING its scan into @czap/audit (existing — already its normalizeRepoPath home) and having both the CLI host and MCP host provide the capability (injected-capability pattern). No new package, no shim.
