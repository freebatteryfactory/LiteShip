 ...
 * `LITESHIP_CAPSULE_MANIFEST` (via getCapsuleManifestPath), but the WRITER
 * (scripts/capsule-compile.ts) hardcoded `reports/capsule-manifest.json`, so the
 * write side could not be isolated.
 *
 * This guard pins the writer's env-var honoring: with LITESHIP_CAPSULE_MANIFEST set,
...
describe('capsule manifest isolation (CUT T1)', () => {
  it(
    'capsule:compile writes to LITESHIP_CAPSULE_MANIFEST and leaves the default path untouched',
    async () => {
      const tmpDir = mkdtempSync(join(tmpdir(), 'liteship-t1-'));
      ...
      const priorEnv = process.env.LITESHIP_CAPSULE_MANIFEST;
      const priorManifestOnly = process.env.LITESHIP_CAPSULE_MANIFEST_ONLY;
      process.env.LITESHIP_CAPSULE_MANIFEST = isolatedManifest;
      ...
      try {
        ...
        expect(existsSync(isolatedManifest), 'manifest must be written to LITESHIP_CAPSULE_MANIFEST path').toBe(true);
        ...
        if (priorManifestOnly === undefined) delete process.env.LITESHIP_CAPSULE_MANIFEST_ONLY;
        else process.env.LITESHIP_CAPSULE_MANIFEST_ONLY = priorManifestOnly;
