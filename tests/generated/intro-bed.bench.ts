...
bench(`intro-bed — decode throughput (budget p95 ${String(cap.budgets.p95Ms ?? 'n/a')}ms)`, async () => {
  if (fixtureBytes === undefined) {
    throw new Error(
      'intro-bed: canonical fixture missing at ' + fixtureAbs + ' — restore examples/scenes/intro-bed.wav (or fix the asset decl source) and re-run pnpm run capsule:compile',
    );
  }
  if (cap.derive === undefined) {
    throw new Error(
      'intro-bed: capsule has no derive handler — defineAsset should resolve decl.decoder ?? builtinDecoderFor(kind); check packages/assets/src/contract.ts and re-run pnpm run capsule:compile',
    );
  }
