 ...
 * `data-czap-directive` marker and the bootstrap script; this spec proves
 * the part that was inert downstream: a real browser loads the built page,
 * the scanner activates the Satellite element, and `data-czap-state`
...
test.describe('astro directive boot (built example)', () => {
  ...
  test('the boot scanner activates the Satellite and data-czap-state tracks viewport.width', async ({ page }) => {
    await page.setViewportSize({ width: 1400, height: 800 });
    await page.goto('/astro-example/');

    const satellite = page.locator('[data-czap-directive~="satellite"]');
    await expect(satellite).toHaveCount(1);

    // Activation is the regression: 0.1.4 shipped this exact page shape inert.
    await expect(satellite).toHaveAttribute('data-czap-state', 'full');

    await page.setViewportSize({ width: 900, height: 800 });
    await expect(satellite).toHaveAttribute('data-czap-state', 'medium');

    await page.setViewportSize({ width: 500, height: 800 });
    await expect(satellite).toHaveAttribute('data-czap-state', 'compact');

    // The plain data-czap-boundary div carries no directive marker and no
    // client:* attribute — the scanner must leave it alone.
    const plain = page.locator('[data-czap-boundary]:not([data-czap-directive])');
    await expect(plain).toHaveCount(1);
    await expect(plain).not.toHaveAttribute('data-czap-state');
  });
});
