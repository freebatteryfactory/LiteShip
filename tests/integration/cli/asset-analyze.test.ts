...
describe('liteship asset analyze', () => {
  ...
  it(
    'runs beat projection on intro-bed and emits markerCount',
    async () => {
      ...
      const { exit, stdout } = await captureCli(() => run(['asset', 'analyze', 'intro-bed', '--projection=beat']));
      expect(exit).toBe(0);
  ...
  it('exits 1 for unknown asset', async () => {
    const { exit } = await captureCli(() => run(['asset', 'analyze', 'missing-asset-12345', '--projection=beat']));
    expect(exit).toBe(1);
  });
  ...
  it(
    'runs onset projection and emits markerCount; the second call comes from cache',
    async () => {
  ...
  it(
    'runs waveform projection and emits markerCount (covers the else arm)',
    async () => {
