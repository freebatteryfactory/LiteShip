...
const namespaceArb = fc.stringMatching(/^[a-z][a-z0-9.-]{0,15}$/u).filter((namespace) => !reserved.has(namespace));
...
describe('downstream gate identity', () => {
  it('admits any valid foreign namespace only with exact owner metadata', () => {
    fc.assert(
      fc.property(
        namespaceArb,
        fc.string({ minLength: 1, maxLength: 40 }).filter((owner) => owner.trim() !== ''),
        (namespace, owner) => {
