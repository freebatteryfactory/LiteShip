...
import { relativePhysicalPath, type PhysicalPathSemantics } from '../../packages/cli/src/internal/physical-path.js';

describe('physical path identity properties', () => {
  test('alias spelling cannot change containment or the relative physical path', () => {
    fc.assert(
      fc.property(fc.array(fc.stringMatching(/^[A-Za-z0-9 _-]{1,12}$/), { minLength: 1, maxLength: 5 }), (segments) => {
