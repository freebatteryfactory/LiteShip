...
describe('SyncSystem (world-query path)', () => {
  it('pulses intensity to ~1 on the frame of a beat', () => {
    ...
    world.spawn({ Beat: { _tag: 'beat', timeMs: 0, strength: 1 } });
    world.spawn({ Beat: { _tag: 'beat', timeMs: 500, strength: 1 } });
    world.spawn({ Beat: { _tag: 'beat', timeMs: 1000, strength: 1 } });
    // frameIndex=30 at 60fps → currentTimeMs = 500 → lastBeat = 500 → exp(0) = 1.
    world.addSystem(SyncSystem(30, 60));
    world.tick();
    const fx = world.query('SyncAnchor');
  ...
  it('writes intensity = 0 when no beats have occurred yet (lastBeat = -Infinity → exp(-Inf) = 0)', () => {
    ...
    world.addSystem(SyncSystem(30, 60)); // currentTimeMs = 500
    world.tick();
    const fx = world.query('SyncAnchor');
    const intensity = fx[0]?.components.get('_intensity');
  ...
  describe('FrameRange gating of the envelope contribution', () => {
    ...
    it('writes plain decay (no envelope factor) when frameIndex is before the FrameRange', () => {
      // Frame 30 < from(40): dormant effect. Pre-fix the pulse leaked a
      // factor of 1 + 0.5·(1 − 20/30) ≈ 1.1667; the gate restores 1.
      expect(intensityAtFrame30({ from: 40, to: 60 })).toBeCloseTo(1, 5);
    });

    it('writes plain decay (no envelope factor) when frameIndex is after the FrameRange', () => {
      // Frame 30 ≥ to(20): effect window already over.
      expect(intensityAtFrame30({ from: 0, to: 20 })).toBeCloseTo(1, 5);
    });

    it('composes decay × envelope exactly at range.from (inclusive lower bound, EffectSystem idiom)', () => {
      // Frame 30 === from: in range, pulse phase 0 → factor 1.5.
      expect(intensityAtFrame30({ from: 30, to: 60 })).toBeCloseTo(1.5, 5);
    });

    it('writes plain decay exactly at range.to (exclusive upper bound, EffectSystem idiom)', () => {
      // Frame 30 === to: out of range per the half-open window. Pre-fix
      // the pulse peaked here (phase 0 → 1.5); the gate restores 1.
      expect(intensityAtFrame30({ from: 0, to: 30 })).toBeCloseTo(1, 5);
    });
  });

  it('skips Beat entities whose Beat component is missing or has a non-numeric timeMs', () => {
    ...
    world.spawn({ Beat: { _tag: 'beat', timeMs: 'oops', strength: 1 } });
