...
describe('Receipt', () => {
  describe('chain validation', () => {
    test('buildChain produces valid chain that passes validateChain', async () => {
      const chain = await Receipt.buildChain(makeEntries(5));
    ...
    test('consecutive envelopes link via previous hash', async () => {
      const chain = await Receipt.buildChain(makeEntries(3));
    ...
    test('validateChainDetailed returns true for valid chain', async () => {
      const chain = await Receipt.buildChain(makeEntries(3));
      const result = await Receipt.validateChainDetailed(chain);
      expect(result).toBe(true);
    });

    test('validateChainDetailed accepts an empty chain', async () => {
      await expect(Receipt.validateChainDetailed([])).resolves.toBe(true);
    });
    ...
    test('validateChainDetailed detects hash mismatch', async () => {
      ...
      const result = await rejection<ChainValidationError>(Receipt.validateChainDetailed(tampered));
      expect(result.type).toBe('hash_mismatch');
      if (result.type === 'hash_mismatch') expect(result.index).toBe(1);
    });
    ...
    test('validateChainDetailed detects chain break', async () => {
      ...
      const detached = await Receipt.createEnvelope(
        detachedEntry.kind,
      ...
      const result = await rejection<ChainValidationError>(Receipt.validateChainDetailed(broken));
      expect(result.type).toBe('chain_break');
      if (result.type === 'chain_break') expect(result.index).toBe(2);
    });

    test('validateChainDetailed detects non-genesis roots', async () => {
      const hlc = HLC.increment(HLC.create('node-a'), 1000);
      const envelope = await Receipt.createEnvelope('test', subject('actor-1'), makePayload(), hlc, 'not-genesis');

      const result = await rejection<ChainValidationError>(Receipt.validateChainDetailed([envelope]));
    ...
    test('validateChainDetailed detects non-increasing HLC values', async () => {
      const entries = makeEntries(3);
      const chain = await Receipt.buildChain(entries);
      const stale = await Receipt.createEnvelope(
        chain[2]!.kind,
        chain[2]!.subject,
        chain[2]!.payload,
        chain[0]!.timestamp,
      ...
      const result = await rejection<ChainValidationError>(
        Receipt.validateChainDetailed([chain[0]!, chain[1]!, stale]),
      );
    ...
    test('validateChain rejects hash mismatches, chain breaks, and non-increasing HLC values', async () => {
      ...
      const tampered = [...chain];
      tampered[1] = { ...tampered[1]!, hash: 'tampered-hash' };
      await expect(Receipt.validateChain(tampered)).rejects.toThrow(/hash mismatch/);
      ...
      await expect(Receipt.validateChain([chain[0]!, chain[1]!, detached])).rejects.toThrow(/chain break/);
      ...
      await expect(Receipt.validateChain([chain[0]!, chain[1]!, stale])).rejects.toThrow(
        /HLC not monotonically increasing/,
      );
    });

    test('empty chain validates successfully', async () => {
      const valid = await Receipt.validateChain([]);
      expect(valid).toBe(true);
    });
    ...
    test('validateChain accepts merge envelopes without forcing linear previous or HLC monotonicity', async () => {
      ...
      await expect(Receipt.validateChain([chain[0]!, mergeEnvelope])).resolves.toBe(true);
    });
  ...
  describe('head / tail / find', () => {
    test('head returns last envelope, tail returns first', async () => {
      const chain = await Receipt.buildChain(makeEntries(4));
  ...
  describe('append', () => {
    test('append extends chain and preserves validity', async () => {
      const chain = await Receipt.buildChain(makeEntries(2));
      ...
      const valid = await Receipt.validateChain(extended);
      expect(valid).toBe(true);
    });

    test('append to empty chain uses genesis', async () => {
      const hlc = HLC.increment(HLC.create('node-a'), 1000);
      const chain = await Receipt.append([], {
        kind: 'first',
        subject: subject('actor-1'),
        payload: makePayload(),
        timestamp: hlc,
  ...
  describe('MAC verification', () => {
    test('sign and verify MAC roundtrip succeeds', async () => {
      ...
      const signed = await Receipt.macEnvelope(chain[0]!, key);
      expect(signed.signature).toBeDefined();
      expect(typeof signed.signature).toBe('string');
      expect(signed.signature!.length).toBeGreaterThan(0);
      const verified = await Receipt.verifyMAC(signed, key);
    ...
    test('unsigned envelope fails MAC verification', async () => {
      const chain = await Receipt.buildChain(makeEntries(1));
      const key = await Receipt.generateMACKey();
      const verified = await Receipt.verifyMAC(chain[0]!, key);
      expect(verified).toBe(false);
    });

    test('MAC with wrong key fails verification', async () => {
      const chain = await Receipt.buildChain(makeEntries(1));
      const key1 = await Receipt.generateMACKey();
      const key2 = await Receipt.generateMACKey();
      const signed = await Receipt.macEnvelope(chain[0]!, key1);
      const verified = await Receipt.verifyMAC(signed, key2);
      expect(verified).toBe(false);
    });
    ...
    test('verifyMAC with empty signature returns false', async () => {
      ...
      const verified = await Receipt.verifyMAC(envelope, key);
      expect(verified).toBe(false);
    });
    ...
    test('generateMACKey surfaces crypto generation failures with wrapped context', async () => {
      ...
      try {
        await expect(Receipt.generateMACKey()).rejects.toThrow(/Failed to generate MAC key/);
      } finally {
    ...
    test('macEnvelope surfaces crypto signing failures with wrapped context', async () => {
      ...
      try {
        await expect(Receipt.macEnvelope(chain[0]!, key)).rejects.toThrow(/Failed to MAC envelope/);
      } finally {
    ...
    test('verifyMAC surfaces crypto verification failures with wrapped context', async () => {
      ...
      try {
        await expect(Receipt.verifyMAC(signed, key)).rejects.toThrow(/Failed to verify signature/);
      } finally {
  ...
  describe('multi-parent merge receipts', () => {
    test('createEnvelope with array previous produces merge envelope', async () => {
      const chain1 = await Receipt.buildChain(makeEntries(2));
    ...
    test('append with explicit previousHashes creates merge receipt', async () => {
      const chain = await Receipt.buildChain(makeEntries(2));
      const otherChain = await Receipt.buildChain(
        makeEntries(1).map((e) => ({
          ...e,
          subject: subject('actor-2'),
      ...
      const merged = await Receipt.append(
        chain,
    ...
    test('hashEnvelope normalizes merge parent ordering and genesis arrays are recognized', async () => {
      ...
      const unsortedHash = await Receipt.hashEnvelope(unsorted);
      const sortedHash = await Receipt.hashEnvelope(sorted);
