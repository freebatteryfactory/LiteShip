...
import { TypeValidator, schema } from '@liteship/core';
...
describe('TypeValidator', () => {
  it('returns an ok result carrying the decoded value on a match', () => {
    ...
    const result = TypeValidator.validate(schema.number, 42);
  ...
  it('returns a tagged ParseError result on a schema mismatch (never throws)', () => {
    const result = TypeValidator.validate(schema.number, 'not a number');
