 ...
 * a provisional `data-czap-tier`. The deferred GPU-probe upgrade script
 * (`emitDetectUpgradeScript`) later REWRITES `data-czap-tier` with the real GPU
 ...
 * hand-rolled inline ladder in `@czap/astro`'s integration.
 *
 ...
 * `data-czap-tier` equals canonical `capTierFromCapabilities` on the SAME
 ...
 * matchMedia, and return the provisional `data-czap-tier` it writes. This is the
...
function runProvisional(input: {
  ...
  try {
    ...
    return {
      ...
      provisional: document.documentElement.getAttribute('data-czap-tier-provisional'),
    };
  } finally {
...
afterEach(() => {
  ...
  document.documentElement.removeAttribute('data-czap-reduced-motion');
  document.documentElement.removeAttribute('data-czap-scheme');
  document.documentElement.removeAttribute('data-czap-touch');
});

describe('provisional detect script is a derived artifact of canonical @czap/detect', () => {
  ...
  test('navigator defaults (cores=2, mem=4) still mirror canonical', () => {
    ...
    try {
      ...
      expect(document.documentElement.getAttribute('data-czap-tier')).toBe(expected);
    } finally {
      Object.defineProperty(window, 'matchMedia', { configurable: true, value: realMatchMedia });
    }
  });
