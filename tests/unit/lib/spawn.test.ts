...
describe('spawnArgvCapture', () => {
  it('drains a failing child beyond 16 MiB while retaining only the true bounded tail', async () => {
    const result = await spawnArgvCapture(
      ...
      [
        '-e',
        `process.stdout.write('x'.repeat(${17 * 1024 * 1024})); process.stdout.write('ACTUAL_TAIL'); process.exit(7);`,
      ],
      { captureBytes: 4_096 },
    );
    expect(result.exitCode).toBe(7);
    expect(result.stdout).toHaveLength(4_096);
    expect(result.stdout).toMatch(/ACTUAL_TAIL$/u);
  });
});
...
describe('withSpawned lifecycle', () => {
  ...
  it('disposes the child even when the callback throws', async () => {
    await expect(
      withSpawned('node', ['-e', 'setInterval(() => {}, 1000)'], async () => {
  ...
  it('dispose is idempotent if the child has already exited', async () => {
    await withSpawned('node', ['-e', 'process.exit(0)'], async (handle) => {
  ...
  it(
    'dispose reaps the entire process tree, not just the immediate child',
    async () => {
