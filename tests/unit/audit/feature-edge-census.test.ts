...
describe('checker-backed feature-edge census', () => {
  it('replays the escaped MotionProgram orphan while classifying VideoSource as connected', () => {
  ...
  it('mints one deterministic CurePacket from the retained real defect', () => {
    const { defect, facts } = historicalFacts();
  ...
  it('is order-independent and self-proves the pure gate', () => {
    const { defect, facts } = historicalFacts();
  ...
  it('reports dynamic canonical edges as unknown and ignores unrelated same-name methods', () => {
