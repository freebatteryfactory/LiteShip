...
describe('consumer project package-manager authority', () => {
  it('prefers authored packageManager metadata over lockfiles and invocation environment', () => {
    ...
    expect(detectProjectPackageManager(root, { npm_config_user_agent: 'pnpm/10.32.1 node/v22' })).toBe('npm');
  });
  ...
  it('uses the one unambiguous project lockfile before the invoking user agent', () => {
    ...
    expect(detectProjectPackageManager(pnpmRoot, { npm_config_user_agent: 'npm/10.9.2 node/v22' })).toBe('pnpm');

    const npmRoot = fixture();
    writeFileSync(join(npmRoot, 'package-lock.json'), '{}\n');
    expect(detectProjectPackageManager(npmRoot, { npm_config_user_agent: 'pnpm/10.32.1 node/v22' })).toBe('npm');
  });

  it('uses the invoking manager for an unmarked project and otherwise defaults to npm', () => {
    const root = fixture();
    expect(detectProjectPackageManager(root, { npm_config_user_agent: 'pnpm/10.32.1 node/v22' })).toBe('pnpm');
    expect(detectProjectPackageManager(root, {})).toBe('npm');
  });
