...
describe('sceneDevSetup', () => {
  it('returns error 1 with a structured stderr event for a missing scene file', async () => {
    const { result, stderr } = await captureStdout(() => sceneDevSetup('does/not/exist.ts'));
  ...
  it(
    'boots a real Vite server for an existing scene path and emits the receipt',
    async () => {
      const { result, stdout } = await captureStdout(() => sceneDevSetup('examples/scenes/intro.ts'));
  ...
  it(
    'sceneDev: full SIGINT-await loop resolves with 0 when a SIGINT is delivered',
    async () => {
