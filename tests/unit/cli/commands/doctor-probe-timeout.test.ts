 ...
 *
 * It lives in its OWN file because it injects a timed-out spawn to force the
 * timeout path — the main doctor.test.ts keeps probing the LIVE workspace, unmocked.
 *
 * @module
 */
...
// Force every external probe spawn down the timed-out path via doctor's injectable
// spawn seam. Sync probes (node, workspace, built, hooks, playwright) still run live
// against the real workspace; no --fix, so the visible-spawn fix path is never reached.
const timedOutSpawn = vi.fn(async () => ({ exitCode: 124, stdout: '', stderr: '', timedOut: true }));
const timedOutDeps = { spawn: timedOutSpawn };
...
describe('doctor — timed-out external probe → warn, bounded (CUT test-flake)', () => {
  it('completes (no hang) and emits the normal receipt shape when every spawn times out', async () => {
    const { stdout } = await captureCli(() => doctor({ pretty: false }, timedOutDeps));
