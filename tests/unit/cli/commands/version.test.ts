/**
 * Unit tests for `liteship version`. Emits a JSON receipt with liteship, Node,
...
describe('version command', () => {
  ...
  it('emits a receipt with liteship + node versions', async () => {
    ...
    expect(receipt.liteship).toMatch(/^\d+\.\d+\.\d+/);
  ...
  it('pretty mode writes a one-liner to stderr', async () => {
    ...
    expect(exit).toBe(0);
    // Format: `liteship <semver>  (Node <ver>, pnpm <ver-or-not-found>)\n`
    expect(stderr).toMatch(/^liteship \d+\.\d+\.\d+/);
