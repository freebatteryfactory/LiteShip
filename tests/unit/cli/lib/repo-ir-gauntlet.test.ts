...
describe('buildRepoIRForRepo — the host-injected IR build', () => {
  ...
  it('materializes a real IR over the fixture corpus (every source file lands in the table)', () => {
  ...
  it('carries the host regex oracle facts (invariant-regex) alongside the AST oracle facts', () => {
  ...
  it('is DETERMINISTIC — the same source bytes yield an identical IR file table + digests', () => {
  ...
  it('the --symbols build (withSymbolReferences) still produces a faithful IR over the same corpus', () => {
...
describe('runGauntletWithRepoIR — build IR + always-on facts + run + receipt', () => {
  it('runs the default --ir path and returns a well-formed GauntletResult (no opt-ins)', async () => {
  ...
  it('is DETERMINISTIC against the injected clock — two runs over the same bytes agree on the verdict', async () => {
    const root = freshFixture();
  ...
  it('the --no-cache path disarms the verdict cache yet returns the same verdict shape', async () => {
    const root = freshFixture();
  ...
  it('honours an explicit glob override (the non-sentinel forward path)', async () => {
    const root = freshFixture();
    const result = await runGauntletWithRepoIR(root, NOW, ['packages/**/*.ts'], { noCache: true });
    expect(typeof result.blocked).toBe('boolean');
  }, HEAVY);

  it('the --proof opt-in composes its gate + facts without throwing (light path)', async () => {
    const root = freshFixture();
    const result = await runGauntletWithRepoIR(root, NOW, undefined, { cacheCwd: root, withProof: true });
    expect(Array.isArray(result.findings)).toBe(true);
  }, HEAVY);

  it('the --composition opt-in composes its gate + facts without throwing (light path)', async () => {
    const root = freshFixture();
    const result = await runGauntletWithRepoIR(root, NOW, undefined, { cacheCwd: root, withComposition: true });
    expect(Array.isArray(result.findings)).toBe(true);
  }, HEAVY);

  it('the --taint opt-in traces the corpus dataflow and composes the taint gate', async () => {
    const root = freshFixture();
    const result = await runGauntletWithRepoIR(root, NOW, undefined, { cacheCwd: root, withTaint: true });
    expect(Array.isArray(result.findings)).toBe(true);
  }, HEAVY);

  it('the --simulate opt-in drives the determinism corpus and composes the simulation gate', async () => {
    const root = freshFixture();
    const result = await runGauntletWithRepoIR(root, NOW, undefined, { cacheCwd: root, withSimulate: true });
    expect(Array.isArray(result.findings)).toBe(true);
  }, HEAVY);

  it('the cache is NAMESPACED by --symbols — a symbols-on verdict is computed independently', async () => {
  ...
  it('the --supply-chain opt-in (with a real lockfile + workspace) computes the SBOM facts and folds the gate', async () => {
...
describe('runGauntletWithRepoIR — the --mutate / --mcdc seam paths (no L4 seams present)', () => {
  it('--mutate composes the mutation gate; an absent baseline/registry yields an empty (no-floor) run', async () => {
  ...
  it('--mcdc composes the MC/DC gate over the same empty-seam path', async () => {
    const root = freshFixture();
    const result = await runGauntletWithRepoIR(root, NOW, undefined, { cacheCwd: root, withMcdc: true });
    expect(Array.isArray(result.findings)).toBe(true);
  }, HEAVY);

  it('a present, POPULATED mutation-score baseline + equivalents registry parse cleanly (the happy-read path)', async () => {
...
describe('runGauntletWithRepoIR — fail-loud edges (tagged throws, never a silent pass)', () => {
  it('--supply-chain with NO pnpm-lock.yaml throws a tagged InvariantViolation', async () => {
  ...
  it('a corrupt (non-object) committed mutation-score ratchet throws a tagged error on the run', async () => {
    ...
    await expect(
      runGauntletWithRepoIR(root, NOW, undefined, { noCache: true, withMutate: true }),
    ).rejects.toSatisfy((e: unknown) => isTaggedError(e));
  }, HEAVY);

  it('a non-numeric mutation-score entry throws a tagged error on the run (a corrupt ratchet)', async () => {
    const root = freshFixture();
    mkdirSync(join(root, 'benchmarks'), { recursive: true });
    writeFileSync(
  ...
  it('a malformed equivalent-mutant registry (no "entries" array) throws a tagged error on --mutate', async () => {
