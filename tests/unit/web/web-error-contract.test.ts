  ...
  test('rejection reason names the missing ids and both remedies; event detail carries recovery', async () => {
    ...
    if (result.type === 'rejected') {
      expect(result.rejection.reason).toBe(
        'Morph rejected: elements [cart] were required by a preserve hint but are missing from the new HTML. Keep these data-czap-id elements in the server response, or drop them from the preserve hint.',
      );
    }
