/**
 * `@czap/core` re-export conformance — canonical kernel output matches core shim.
 */

import { describe, it, expect } from 'vitest';
import { AddressedDigest as CoreAddressedDigest, CanonicalCbor as CoreCbor, fnv1aBytes as coreFnv1aBytes } from '@czap/core';
import { AddressedDigest, CanonicalCbor, fnv1aBytes } from '@czap/canonical';

describe('@czap/core canonical shim conformance', () => {
  it('CanonicalCbor.encode matches @czap/canonical byte-for-byte', () => {
    const value = { b: 2, a: 1, nested: { z: 3, y: 2 } };
    expect(CoreCbor.encode(value)).toEqual(CanonicalCbor.encode(value));
  });

  it('fnv1aBytes matches @czap/canonical', () => {
    const bytes = CanonicalCbor.encode({ name: 'accent', category: 'color' });
    expect(coreFnv1aBytes(bytes)).toBe(fnv1aBytes(bytes));
  });

  it('AddressedDigest.of matches @czap/canonical for sha256 and blake3', () => {
