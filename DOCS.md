...
Read [ARCHITECTURE.md](./ARCHITECTURE.md) — "Document graph (the IR)" and "AI projection" — then
...
- the AI proposal envelope (validate before apply, never the reverse)
...
| Where is the typed ECS projection/execution substrate?                       | `packages/core/src/ecs/` (`part.ts`, `dense.ts`, `world.ts`), exported only through the advanced `@liteship/core/ecs` subpath (`definePart`/`admitPart`, declared systems, Part-owned dense writers; ADR-0009)                                        |
| What separates authored motion, its schedule, and runtime execution?         | `TransitionProgram` → `TransitionTimeline` in `packages/core/src/motion/transition-program.ts`; admitted `RuntimeWritePlan` in `motion/runtime-write-plan.ts`; Astro wire envelope `MotionDirectivePayload` in `packages/astro/src/runtime/motion.ts`  |
| Where is the shared offline frame-timing law?                                | `FrameSchedule` / `createFrameSchedule` / `createVideoRenderer` in `packages/core/src/media/video.ts`; Remotion and Stage adapt that timing kernel without owning a second schedule                                                                    |
...
| Where is the AI projection / how is a model proposal validated?              | `packages/core/src/authoring/ai-cast.ts` (`castContext` → `validateGraphPatchProposal` → `applyValidatedPatch`); envelope in `evidence/validated-output.ts` (ADR-0015)                                                                                 |
...
| What is the local workstation accept bar?                                    | Focused deterministic packet proofs, then `pnpm resources:plan` and `pnpm preflight --staged`; reserve `gauntlet:full` for cloud/frozen-head authority and use `pnpm run docs:check:local` only when docs inputs require it (see SKILL.md)             |
...
| Where does gate subject coverage earn authority?                             | `GateProof.subjectCoverage` in `packages/gauntlet/src/gate.ts`; admission and required-opaque blocking in `authority.ts` / `engine.ts`; planted proof in `tests/unit/gauntlet/subject-coverage-authority.test.ts`                                      |
| Where is feature-edge connectivity checked?                                  | source-derived census in `packages/audit/src/feature-edge-census.ts`, facts/gate in `packages/gauntlet/src/facts/feature-edge-facts.ts` + `gates/feature-edge-connectivity.ts`, production host in `packages/cli/src/internal/repo-ir-gauntlet.ts`     |
...
| How do I distinguish observed capability inputs from conservative fallbacks? | `CapabilityTierEvidence` + `requireObserved` in `packages/detect/src/cap-axes.ts`; complete values always render, while trust-bearing callers fail closed on inferred axes                                                                             |
...
| How do I bind a form to the mutation channel?                                | `bindGraphForm` from `@liteship/web` (`packages/web/src/graph-form.ts`; ADR-0031; `examples/06-mutation-roundtrip`)                                                                                                                                    |
