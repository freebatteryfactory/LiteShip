...
The `02_targets/` umbrella is sealed at tag `targets-umbrella-seal`, architecture only, with no children authored. The current phase is the Astro/Vite paired denominator: `astro/` and `vite/` are derived and authored together in one working tree, because the seam between them is the first real test of relation-shaped composition. The required child sequence is `astro/` + `vite/`, then `cloudflare/` — which earns the deferred deployment contract — then the roster seal. A media ecosystem target is deferred and uncounted; video remains a core scene/media obligation plus downstream host encoder work, not a target this phase. No child is authored until its denominator is approved.
