...
The `02_targets/` umbrella is sealed at tag `targets-umbrella-seal`. Its first two children are authored: `astro/` across seven numbered homes and `vite/` across six, with the seam between them proved by a real binding in `verification/probes/`. Neither imports the other; a composition point imports both. The required child sequence continues with `cloudflare/`, which earns the deferred deployment contract and is where the producer-neutral `direct-composition` arm gets exercised, and the roster seals after it. A media ecosystem target is deferred and uncounted; video remains a core scene/media obligation plus downstream host encoder work, not a target this phase. No further child is authored until its denominator is approved.
