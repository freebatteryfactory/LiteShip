...
## Edge delivery (0.8 seams)

**Workers static boundary CSS** ([ADR-0025](./docs/adr/0025-workers-static-assets-boundary-css.md)): precompiled boundary outputs ship as immutable `/_czap/<id>/<hash>.css` Workers Static Assets; SSR selects the tier URL instead of inlining bytes on every request. `serializeBoundaryCss` emits only `CompiledOutputs.css` (law 13 — mirrors are not additive).
