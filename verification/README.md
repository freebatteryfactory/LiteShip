...
| `gates/authority.mjs` | One declaration per concept. No nominal tag appears in two homes, and a name may repeat across homes only when each declaration carries its own distinct tag. This is the only check that can see a structurally identical local twin of an imported type — to the compiler those are the same type, so no law can. |
| `gates/authority.selftest.mjs` | The authority gate can fail: a `SourceRelation` twin, a divergent twin, a shared brand, and a shared reference kind are all refused, while realm-scoped brands sharing a name pass. |
