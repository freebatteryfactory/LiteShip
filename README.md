...
Pulled from the `truth-artifacts-linux` artifact of [CI run 28413170218](https://github.com/freebatteryfactory/LiteShip/actions/runs/28413170218) (commit `eef9b38`) on 2026-06-30 (linux x64, Node 22). Refresh with `pnpm exec tsx scripts/refresh-bench-snapshot.ts` against a newer artifact, then `pnpm run docs:gen`.

- `pnpm run gauntlet:full` passed end-to-end in 14m52s under CI conditions.
...
| --- | ---: | ---: | ---: | ---: |
| `satellite` hot path | 2,655ns | 2,573ns | 3.19% | 15% |
| `stream` parse + patch | 748,310ns | 743,316ns | 0.67% | 15% |
| `llm` text chunk parse | 781,601ns | 752,270ns | 4.04% | 15% |
| `worker` fallback eval | 2,926ns | 2,793ns | 4.76% | 15% |
| `llm-startup-shared` | 70,420ns | 70,879ns | -1.48% | 25% |
| `llm-promoted-startup-shared` | 234,639ns | 245,149ns | -3.46% | 25% |
| `worker-runtime-startup-shared` | 1,213ns | 3,528ns | -68.03% | 25% |

Diagnostic watch, not a release gate: `llm-runtime-steady` runs above its relative baseline (p99 ratio 36.60x), but the absolute directive p99 is 380,229ns against a 1,000,000ns steady-state budget — ~38.0% of budget, headroom not panic. The live ledger is the `truth-artifacts-linux` CI artifact (`benchmarks/directive-gate.json`, `gauntlet-phase-timings.json`); see [STATUS.md](./STATUS.md) for the operator view.
<!-- END BENCH -->
