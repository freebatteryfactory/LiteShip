...
The root Type ABI does not stop at a digest-shaped token. It defines the normalized graph that `system/assurance` must produce.
...
`system/assurance` later implements the canonicalizer, computes the surface digest and final address with core's byte/digest capabilities, and verifies the attestation. The address never lives inside the file or surface it identifies.
...
`system/assurance` parses declarations with the root-governed semantic interpreter lane and lowers each observed public type surface into the `TypeAbiSurface` model already declared by `types.d.ts`.
...
Type ABI surfaces and attestations are generated by `system/assurance` using core canonical-byte and digest capabilities once those exist.
...
The current phase is `01_hosts/` closure QA, architecture only. The umbrella contract — including the grounding and admission boundary that distinguishes an authority a host admits from one it constructs — is confirmed and closed, and its child roster is sealed. All four children are specified: `web/` across twelve numbered homes, `worker/` across seven, `edge/` across eleven, and `server/` across eleven. No runtime host implementation is authorized: every home in the repository receives its architecture before any executable product code exists, and implementation opens only when the whole repository architecture closes and Eassa explicitly authorizes it.
...
```yaml
artifact:
  name: liteship-root-architecture-and-type-foundation
  status: settled
  authority:
    architecture: README.md
    type_abi: types.d.ts
  next_phase: host-layer-closure-then-targets-wires-system
core:
  architecture: 00_core/README.md
  topology: 00_core/types.ts
  implementation: absent
  status: confirmed-closed
hosts:
  architecture: 01_hosts/README.md
  topology: 01_hosts/types.ts
  child_homes:
    web: architecture-specified
    worker: architecture-specified
    edge: architecture-specified
    server: architecture-specified
  implementation: absent
  status: umbrella-confirmed-closed
root:
  roles:
  - meta-package
  - public-export-membrane
  - canonical-typescript-toolchain
  - declaration-type-abi
  ...
  type_abi:
    ...
    ambient_globals: false
    runtime_values: false
    separate_package: false
    bare_liteship_declaration_entry: false
    consumer_delivery: transitive-through-owner-declarations
    law_fixtures:
      path: types.laws.ts
      required_to_typecheck: true
      runtime_values: false
      in_abi_population: false
      declaration_emit: false
      public_export: false
      vacuity_proof: reverse-the-guard-and-confirm-the-named-law-fails
  typescript_toolchain:
    authority: root-role-matrix
    independent_home_selection: false
    initial_roles:
      primary-check: typescript-7-native
      compatibility-check: typescript-6-for-embedded-language-parity
      declaration-emit: typescript-7-native
      semantic-abi: typescript-6-compiler-api-until-native-api
      analysis-api: typescript-6-compiler-api-until-native-api
      embedded-language: typescript-6-until-host-support
      language-service: typescript-6-until-astro-embedded-support
    exact_versions_live_in: package-metadata-and-toolchain-fingerprints
waterfall:
  prelude:
  - types.d.ts
  numbered:
  - path: 00_core
    role: complete-realm-neutral-semantics
    upstream:
    - types.d.ts
  - path: 01_hosts
  ...
  - band: 2
    peers:
    - path: 02_targets
      role: ecosystem-lifecycle-integration
    - path: 02_wires
      role: protocol-and-invocation-projection
    upstream:
    - types.d.ts
    - 00_core
    - relevant-01_hosts
  ...
  - path: system
    ...
    - 02_targets
    - 02_wires
    product_homes_may_depend_on_system: false
shape_calculus:
  declaration_style: external-module-d-ts
  families:
    data_algebra:
    - Product
    - Coproduct
    ...
    - RebindPort
    - RefinePort
    waterfall_composition:
    - Extend
    - InvalidRefinementKeys
    - Refine
    - Compose
    ...
    - TypeAbiExport
    - TypeAbiCoverage
    - TypeAbiSurfaceDigest
    - TypeAbiAddress
    - TypeScriptCompilerFingerprint
    - TypeScriptToolchainFingerprint
    - TypeScriptToolchainMatrix
    - TypeAbiAttestation
  semantic_nouns_reserved_for_core:
  - EntityId
  - ContentAddress
  - SurfacePath
  - Schema
  ...
  - Finding
composition:
  broad_type_files: true
  scoped_runtime_files: true
  local_type_file_convention: types.ts
  upstream_redeclaration: forbidden-by-ownership
  additive_composition: Extend
  explicit_specialization: Refine
  specialization_rejects:
  - foreign-keys
  - widening
  - required-to-optional-weakening
  canonical_requirement_form: exact-named-readonly-tuple
  requirement_identity: stable-string-name
  open_requirement_arrays: false
  ergonomic_requirement_view: derived-name-indexed-context
  duplicate_requirement_names: rejected
  cross_layer_requirement_merge: equal-dedupe-new-append-conflict-reject
  runtime_dependency_resolution: by-hole-identity-not-tuple-position
  signature_composition: output-to-input-plus-failure-union-plus-requirement-merge
  imports_define_ownership: true
  holes_define-required-authority: true
  holes_replace-imports: false
  port_narrowing: RefinePort
  port_transform: RebindPort
  binding_rows: closed-and-duplicate-rejecting
  residual_requirements_derive_runtime_features: true
  empty_residual_requirements_emit_runtime: false
  unresolved_required_holes_block_production: true
  fixed_hydration_tier_table: false
imports_and_exports:
  import_from_owner: true
  export_from_owner: true
  compose_locally: true
  transitive-relay-reexports: false
  source-self-import-through-dist: false
  public_visibility: package-exports-whitelist
  declaration_layout_follows-runtime-module-layout: true
content_addressed_type_abi:
  canonicalizer: liteship.type-abi/v1
  canonical_encoding: canonical-cbor
  digest: sha256
  raw_text_hash: false
  canonical_ir: required
  address_scope_initially: public-layer-surface
  interpretation_context:
  - semantic-interpreter-package-version-and-artifact
  - module-resolution
  - compiler-options-that-change-semantics
  - standard-library-fingerprints
  recursive_types: strongly-connected-component-canonicalization
  downstream_address_commits_to-upstream-addresses: true
  address_embedded_in_types_dts: false
  produced_by: system/assurance
  core-address-kernel-consumer: true
  purposes:
  - cache-invalidation
  - declaration-drift
  - release-compatibility-analysis
  - authority-discovery
  - agent-context-fingerprinting
  - packed-artifact-verification
  - assurance-attestations
  compatibility_verdict_separate_from-address-equality: true
  canonical_surface_type: TypeAbiSurface
  normalized_node_vocabulary: TypeAbiNode
  public_population_independent_from_lowering: true
  reachable_private_support_closure: true
  unsupported_forms: explicit-incomplete-coverage
  surface_digest: sha256-over-canonical-TypeAbiSurface
  final_address: sha256-over-canonicalizer-plus-surface-digest-plus-interpreter-address
  interpreter_attestation_lane: semantic-abi
  other_toolchain_lanes: independent-qualification-evidence
  non_semantic_source_details_excluded:
  - comments
  - whitespace
  - source-offsets
  - parameter-names
  - tuple-labels
  - harmless-private-support-names
runtime:
  types_dts_runtime: none
  realm_neutral_runtime_owner: 00_core
  environment_effect_owners: 01_hosts
  ecosystem_integration_owners: 02_targets
  protocol_owners: 02_wires
  repository_program_owner: system
  root_runtime_role: bootstrap-only
ai:
  top_level_home: false
  classification: ingress-and-egress-mode
  generic_mechanisms:
  - core
  - relevant-hosts
  provider_and-protocol_adapters:
  - relevant-wires
  - relevant-targets
  agent-orchestration-framework: false
assurance:
  home: system
  audit_role: evidence-acquisition
  gauntlet_role: evidence-evaluation-and-earned-authority
  type_abi_canonicalization: system/assurance
