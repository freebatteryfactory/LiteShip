...
- **compile one definition to CSS / GPU / ARIA** → [`examples/03-cast-aria`](./examples/03-cast-aria) · [PACKAGE-SURFACES.md](./PACKAGE-SURFACES.md)
- **let AI propose UI changes safely** → [`examples/05-ai-patch-refused`](./examples/05-ai-patch-refused)
- **accept validated edits back from the client** → [`examples/06-mutation-roundtrip`](./examples/06-mutation-roundtrip)
...
|---|---|
| [`tutorial`](./examples/tutorial) | The guided five-page intro: boundaries → tokens → themes → streaming → LLM/genui |
...
| [`06-mutation-roundtrip`](./examples/06-mutation-roundtrip) | The client→server round-trip via `createGraphMutationClient` + `bindGraphForm` — the server validates + applies (stale-base patches refused with auto-recovery); the return leg of the stream |
