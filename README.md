...
Pulled from the `truth-artifacts-linux` artifact of [CI run 28268276811](https://github.com/freebatteryfactory/LiteShip/actions/runs/28268276811) (commit `5f3d76f`) on 2026-06-26 (linux x64, Node 22). Refresh with `pnpm exec tsx scripts/refresh-bench-snapshot.ts` against a newer artifact, then `pnpm run docs:gen`.

- `pnpm run gauntlet:full` passed end-to-end in 40m7s under CI conditions.
...
| --- | ---: | ---: | ---: | ---: |
| `satellite` hot path | 6,240ns | 5,979ns | 4.33% | 15% |
| `stream` parse + patch | 1,727,737ns | 1,738,277ns | -0.35% | 15% |
| `llm` text chunk parse | 1,842,488ns | 1,759,985ns | 4.73% | 15% |
| `worker` fallback eval | 6,838ns | 6,660ns | 2.58% | 15% |
| `llm-startup-shared` | 143,557ns | 145,402ns | -1.33% | 25% |
| `llm-promoted-startup-shared` | 392,832ns | 393,929ns | -0.29% | 25% |
| `worker-runtime-startup-shared` | 2,270ns | 5,728ns | -58.69% | 25% |

Diagnostic watch, not a release gate: `llm-runtime-steady` runs above its relative baseline (p99 ratio 20.36x), but the absolute directive p99 is 593,692ns against a 1,000,000ns steady-state budget — ~59.4% of budget, headroom not panic. The live ledger is the `truth-artifacts-linux` CI artifact (`benchmarks/directive-gate.json`, `gauntlet-phase-timings.json`); see [STATUS.md](./STATUS.md) for the operator view.
<!-- END BENCH -->
